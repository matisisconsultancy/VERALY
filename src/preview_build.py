...
(function(){
  ...
  function initReveal(){var track=document.querySelector('.reveal-track');if(!track||track.__i)return;track.__i=1;
    ...
    var cardStart=parseFloat(track.getAttribute('data-cardstart')),hasCardStart=!isNaN(cardStart);
    function cardThresh(k,n){return hasCardStart?(cardStart+(k/n)*(0.98-cardStart)):((k+1)/(n+1));}
  ...
  function initCountUp(){[].slice.call(document.querySelectorAll('[data-count]')).forEach(function(el){if(el.__cu)return;if(el.closest('.plz-track'))return;el.__cu=1;var target=parseFloat(el.getAttribute('data-count'))||0;el.__cuRun=function(){if(scrReduce){el.textContent=String(target);return;}var t0=0;function step(now){if(!t0)t0=now;var p=Math.min(1,(now-t0)/1200),eased=1-Math.pow(1-p,3);el.textContent=String(Math.round(eased*target));if(p<1)requestAnimationFrame(step);}requestAnimationFrame(step);};if(countIO)countIO.observe(el);else el.__cuRun();});}
  ...
  function render(){var route=curRoute();var t=tpl[route]||tpl['/'];app.innerHTML='';app.appendChild(t.content.cloneNode(true));
    ...
    window.scrollTo(0,0);bindDynamic(app);initStepper();initReveal();initFeatureRows();initBlog();initScramble();initCountUp();initPlazos();initViaSticky();initPhases();initJourney();initBooking();}
  ...
  function initJourney(){var js=[].slice.call(document.querySelectorAll('.journey'));if(!js.length)return;js.forEach(function(j){if(j.__jnInit)return;j.__jnInit=1;var phases=[].slice.call(j.querySelectorAll('.jn-phase'));phases.forEach(function(ph,i){var t=ph.querySelector('.jn-toggle');if(t)t.addEventListener('click',function(){jnApplyOpen(j,phases,j.__open===i?-1:i);});});jnApplyOpen(j,phases,0);j.__lastCur=0;});function upd(){var vh=window.innerHeight;js.forEach(function(j){var spine=j.querySelector('.jn-spine');var phases=[].slice.call(j.querySelectorAll('.jn-phase'));var n=phases.length;if(!n)return;var sec=j.closest('.jn-sec');var pin=sec&&sec.querySelector('.jn-pin-track');var sticky=sec&&sec.querySelector('.jn-pin-sticky');var cur,prog;if(pin&&sticky&&getComputedStyle(sticky).position==='sticky'){var h=pin.offsetHeight-vh;prog=h>0?(-pin.getBoundingClientRect().top)/h:0;prog=Math.max(0,Math.min(0.999,prog));cur=Math.min(n-1,Math.floor(prog*n));}else{var mid=vh*0.5,r=j.getBoundingClientRect();prog=Math.max(0,Math.min(1,(vh*0.4-r.top)/Math.max(1,r.height)));cur=0;phases.forEach(function(ph,i){if(ph.getBoundingClientRect().top<mid)cur=i;});}if(spine)spine.style.setProperty('--p',prog.toFixed(3));phases.forEach(function(ph,i){ph.classList.toggle('is-on',i<=cur);});if(cur!==j.__lastCur){jnApplyOpen(j,phases,cur);j.__lastCur=cur;}});}if(!window.__jnScroll){window.__jnScroll=1;window.addEventListener('scroll',upd,{passive:true});window.addEventListener('resize',upd);}upd();}
  function initBooking(){var w=document.querySelector('[data-booking]');if(!w)return;var daysEl=w.querySelector('[data-days]'),slotsEl=w.querySelector('[data-slots]'),confirmBtn=w.querySelector('[data-confirm]'),selEl=w.querySelector('[data-selected]'),email=w.getAttribute('data-email')||'';var active=w.querySelector('.cal-chip.is-active');var state={motivo:active?active.getAttribute('data-motivo'):'',day:null,dayLabel:'',time:null};[].slice.call(w.querySelectorAll('.cal-chip')).forEach(function(c){c.addEventListener('click',function(){[].slice.call(w.querySelectorAll('.cal-chip')).forEach(function(x){x.classList.remove('is-active');});c.classList.add('is-active');state.motivo=c.getAttribute('data-motivo');});});var dows=['domingo','lunes','martes','miércoles','jueves','viernes','sábado'],months=['ene','feb','mar','abr','may','jun','jul','ago','sep','oct','nov','dic'];var d=new Date();d.setHours(0,0,0,0);var added=0;while(added<10){d.setDate(d.getDate()+1);var dow=d.getDay();if(dow===0||dow===6)continue;var lbl=dows[dow]+' '+d.getDate()+' '+months[d.getMonth()];var b=document.createElement('button');b.type='button';b.className='cal-day';b.innerHTML='<span class="cal-dow">'+lbl+'</span>';(function(lbl2){b.addEventListener('click',function(){[].slice.call(w.querySelectorAll('.cal-day')).forEach(function(x){x.classList.remove('is-active');});b.classList.add('is-active');state.day=lbl2;state.dayLabel=lbl2;state.time=null;buildSlots();upd();});})(lbl);daysEl.appendChild(b);added++;}function buildSlots(){slotsEl.innerHTML='';[8,9,10,11,12,14,15,16,17].forEach(function(h){var t=('0'+h).slice(-2)+':00';var s=document.createElement('button');s.type='button';s.className='cal-slot';s.textContent=t;s.addEventListener('click',function(){[].slice.call(w.querySelectorAll('.cal-slot')).forEach(function(x){x.classList.remove('is-active');});s.classList.add('is-active');state.time=t;upd();});slotsEl.appendChild(s);});}function upd(){var ok=!!(state.day&&state.time);confirmBtn.disabled=!ok;if(ok){selEl.hidden=false;selEl.innerHTML='Franja seleccionada: <b>'+state.dayLabel+' · '+state.time+'</b>';}else selEl.hidden=true;}confirmBtn.addEventListener('click',function(){if(confirmBtn.disabled)return;var nombre=(w.querySelector('[data-f="nombre"]')||{}).value||'',contacto=(w.querySelector('[data-f="contacto"]')||{}).value||'';var subject='Solicitud de cita — '+state.dayLabel+' '+state.time;var body='Motivo: '+state.motivo+'\nFranja solicitada: '+state.dayLabel+' a las '+state.time+'\nNombre: '+nombre+'\nContacto: '+contacto+'\n\n(No incluyo los hechos del caso; los conversamos en la reunión.)';window.location.href='mailto:'+email+'?subject='+encodeURIComponent(subject)+'&body='+encodeURIComponent(body);});}
