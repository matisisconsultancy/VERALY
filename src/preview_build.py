...
(function(){
  ...
  function initFeatureRows(){var rows=[].slice.call(document.querySelectorAll('.frow')).concat([].slice.call(document.querySelectorAll('.prac-rows'))).concat([].slice.call(document.querySelectorAll('.pr-timeline'))).concat([].slice.call(document.querySelectorAll('.reveal-up')));rows.forEach(function(r){if(r.__f)return;r.__f=1;if(frowIO)frowIO.observe(r);else r.classList.add('in');});if(rows.length&&!window.__frowScroll){window.__frowScroll=1;window.addEventListener('scroll',frowPar,{passive:true});window.addEventListener('resize',frowPar);}frowPar();}
  ...
  function render(){var route=curRoute();var t=tpl[route]||tpl['/'];app.innerHTML='';app.appendChild(t.content.cloneNode(true));
    ...
    window.scrollTo(0,0);bindDynamic(app);initStepper();initReveal();initFeatureRows();initBlog();initScramble();}
  ...
  function initScramble(){if(scrReduce)return;[].slice.call(document.querySelectorAll('.eyebrow, .eyebrow-num, .faq-pill, .bfilter-pill, .article-kicker')).forEach(function(el){if(el.__scr)return;el.__scr=1;if(scrIO)scrIO.observe(el);else scrambleText(el);});}
