...
(function(){
  ...
  function initViaSticky(){var track=document.querySelector('.via-track');if(!track)return;var slides=[].slice.call(track.querySelectorAll('.via-slide')),bars=[].slice.call(track.querySelectorAll('.via-bars i')),steps=[].slice.call(track.querySelectorAll('.via-step')),n=slides.length;if(!n)return;function upd(){var h=track.offsetHeight-window.innerHeight;var p=h>0?(-track.getBoundingClientRect().top)/h:0;p=Math.max(0,Math.min(0.999,p));var idx=Math.floor(p*n);slides.forEach(function(s,i){s.classList.toggle('active',i===idx);});bars.forEach(function(b,i){b.classList.toggle('on',i<=idx);});steps.forEach(function(s,i){s.classList.toggle('on',i===idx);});}if(!window.__viaScroll){window.__viaScroll=1;window.addEventListener('scroll',upd,{passive:true});window.addEventListener('resize',upd);}upd();}
  function initPlazos(){var secs=[].slice.call(document.querySelectorAll('.plz-sec'));if(!secs.length)return;function upd(){var vh=window.innerHeight;secs.forEach(function(sec){var track=sec.querySelector('.plz-track');if(!track)return;var steps=[].slice.call(track.querySelectorAll('.plz-step'));var pin=sec.querySelector('.plz-pin-track');var p;if(pin&&getComputedStyle(sec.querySelector('.plz-pin-sticky')).position==='sticky'){var h=pin.offsetHeight-vh;p=h>0?(-pin.getBoundingClientRect().top)/h:0;}else{var r=sec.getBoundingClientRect();p=(vh*0.72-r.top)/Math.max(1,r.height*0.55);}p=Math.max(0,Math.min(1,p));track.style.setProperty('--p',p.toFixed(3));var active=Math.ceil(p*steps.length);steps.forEach(function(s,i){s.classList.toggle('is-on',i<active);s.classList.toggle('is-cur',i===active-1);});});}if(!window.__plzScroll){window.__plzScroll=1;window.addEventListener('scroll',upd,{passive:true});window.addEventListener('resize',upd);}upd();}
  function initJourney(){var js=[].slice.call(document.querySelectorAll('.journey'));if(!js.length)return;function upd(){var vh=window.innerHeight,mid=vh*0.62;js.forEach(function(j){var spine=j.querySelector('.jn-spine');var phases=[].slice.call(j.querySelectorAll('.jn-phase'));var r=j.getBoundingClientRect();var p=(mid-r.top)/Math.max(1,r.height*0.72);p=Math.max(0,Math.min(1,p));if(spine)spine.style.setProperty('--p',p.toFixed(3));phases.forEach(function(ph){var nr=ph.getBoundingClientRect();ph.classList.toggle('is-on',nr.top<mid);});});}if(!window.__jnScroll){window.__jnScroll=1;window.addEventListener('scroll',upd,{passive:true});window.addEventListener('resize',upd);}upd();}
