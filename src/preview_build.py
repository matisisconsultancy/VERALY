...
(function(){
  ...
  function frowPar(){var vh=window.innerHeight;[].slice.call(document.querySelectorAll('.fr-par')).forEach(function(par){var m=par.parentNode.getBoundingClientRect();var rel=((m.top+m.height/2)-vh/2)/vh;rel=Math.max(-1,Math.min(1,rel));var t=(1-rel)/2;var scale=(1+0.24*t).toFixed(3);par.style.transform='translate3d(0,'+(rel*-9).toFixed(2)+'%,0) scale('+scale+')';});
    [].slice.call(document.querySelectorAll('.pr-tl-par')).forEach(function(par){var m=par.parentNode.getBoundingClientRect();var rel=Math.max(-1,Math.min(1,((m.top+m.height/2)-vh/2)/vh));var scale=(1+0.10*((1-rel)/2)).toFixed(3);par.style.transform='translate3d(0,'+(rel*-6).toFixed(2)+'%,0) scale('+scale+')';});
    [].slice.call(document.querySelectorAll('.pr-timeline')).forEach(function(tl){var rows=[].slice.call(tl.querySelectorAll('.pr-tl-row')),best=null,bestD=1e9,c=vh/2;rows.forEach(function(r){var b=r.getBoundingClientRect(),d=Math.abs((b.top+b.height/2)-c);if(d<bestD){bestD=d;best=r;}});rows.forEach(function(r){r.classList.toggle('active',r===best&&bestD<vh*0.5);});});}
  function initFeatureRows(){var rows=[].slice.call(document.querySelectorAll('.frow')).concat([].slice.call(document.querySelectorAll('.prac-rows'))).concat([].slice.call(document.querySelectorAll('.pr-timeline')));rows.forEach(function(r){if(r.__f)return;r.__f=1;if(frowIO)frowIO.observe(r);else r.classList.add('in');});if(rows.length&&!window.__frowScroll){window.__frowScroll=1;window.addEventListener('scroll',frowPar,{passive:true});window.addEventListener('resize',frowPar);}frowPar();}
